...
def simulate_wildfire():
    ...
    for day in range(DAYS):
        ...
        if day % 5 == 0 or day == DAYS - 1:
            ...
            # Save VTK files at output intervals
            vtk_filename = f"frame_{outputCount:03d}.vtk"
            save_particles_to_vtk(vtk_filename, forest )
            outputCount += 1

    
    return fire_spread

def save_particles_to_vtk(filename, positions):
    """
    Save particle positions, velocities, and masses to a VTK file.
    """
    ...
    vtk_data = pyvtk.VtkData(
        pyvtk.StructuredGrid(dimensions=(GRID_SIZE, GRID_SIZE, 1), points=points),
        pyvtk.PointData(
            pyvtk.Scalars(positions.flatten(), name="Forest"),    )
    )
